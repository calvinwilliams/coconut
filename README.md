...
- [1. 概述](#1-概述)
- [2. 编译安装](#2-编译安装)
        - [2.1. 下载最新源码](#21-下载最新源码)
        - [2.2. 编译源码安装](#22-编译源码安装)
- [3. 全局序列号发生器](#3-全局序列号发生器)
        - [3.1. 序列号格式](#31-序列号格式)
        - [3.2. 服务接口](#32-服务接口)
                - [3.2.1. 获取序列号](#321-获取序列号)
                - [3.2.2. 反解序列号](#322-反解序列号)
        - [3.3. 场景示例](#33-场景示例)
                - [3.3.1. 启动服务](#331-启动服务)
                - [3.3.2. 获取序列号](#332-获取序列号)
                - [3.3.3. 反解序列号](#333-反解序列号)
                - [3.3.4. 停止服务](#334-停止服务)
        - [3.4. 性能压测](#34-性能压测)
                - [3.4.1. 获取序列号（短连接）](#341-获取序列号短连接)
- [4. 全局额度管理器](#4-全局额度管理器)
        - [4.1. 使用说明](#41-使用说明)
        - [4.2. 服务接口](#42-服务接口)
                - [4.2.1. 查询额度](#421-查询额度)
        ...
        - [4.3. 场景示例](#43-场景示例)
                ...
                - [4.3.6. 扣减额度](#436-扣减额度)
                - [4.3.7. 清空额度](#437-清空额度)
                - [4.3.8. 查看导出的申请流水文件](#438-查看导出的申请流水文件)
        - [4.4. 性能压测](#44-性能压测)
                - [4.4.1. 申请额度（长连接）](#441-申请额度长连接)
- [5. 附录A.启动命令行参数](#5-附录a启动命令行参数)
- [6. 最后](#6-最后)
...
| 区号 | 区名 | 说明 |
|:---:|:---:| --- |
| 第一区 | 分区目录(index) | 2个六十四进制字符 共12个二进制位<br>第一段3个二进制位表示保留区六十四进制字符个数<br>第二段3个二进制位表示服务器编号区六十四进制字符个数<br>第三段3个二进制位表示秒戳区六十四进制字符个数<br>第四段3个二进制位表示序号区六十四进制字符个数 |
